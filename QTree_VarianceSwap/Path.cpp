...
void Path::buildAndCalc_variance_std(){
	this->buildTree();
	this->calcOptValue_variance_std();
}

void Path::buildAndCalc_variance_sim(){
	this->buildTree();
	this->calcOptValue_variance_sim();
}
...
void Path::calcOptValue_variance_std(){
	vector<TimeSlice*>::reverse_iterator rev_itTSL;
	for (rev_itTSL = TSPtrVec.rbegin()+1; rev_itTSL != TSPtrVec.rend(); rev_itTSL++)
	{
		(*rev_itTSL)->optAllNodes_variance_std();
	}
	vector<TimeSlice*>::iterator itTSL = TSPtrVec.begin();
	list<Node> rootNodeList = (*itTSL)->getNodeList();
	Node rootNode = rootNodeList.front();

	this->optValuePath_variance_std =10000* rootNode.getOpt_variance_std() * 252/(this->pathPara.Steps)
												*exp( - this->pathPara.Interest * this->pathPara.Maturity );

}

void Path::calcOptValue_variance_sim(){
	vector<TimeSlice*>::reverse_iterator rev_itTSL;
	for (rev_itTSL = TSPtrVec.rbegin()+1; rev_itTSL != TSPtrVec.rend(); rev_itTSL++)
	{
		(*rev_itTSL)->optAllNodes_variance_sim();
	}
	vector<TimeSlice*>::iterator itTSL = TSPtrVec.begin();
	list<Node> rootNodeList = (*itTSL)->getNodeList();
	Node rootNode = rootNodeList.front();

	this->optValuePath_variance_sim =10000* rootNode.getOpt_variance_sim()*252/(this->pathPara.Steps)
											*exp( - this->pathPara.Interest * this->pathPara.Maturity );
...
double Path::getOptionValue_variance_std()
{
	return optValuePath_variance_std;
}

double Path::getOptionValue_variance_sim()
{
	return optValuePath_variance_sim;
}
