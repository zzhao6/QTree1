...
void TimeSlice::optAllNodes_variance_std(){
	list<Node>::iterator itNL;
	itNL = NodeList.begin();
	...
	if (tmpNodePtr[0] == NULL)
	{
		for (itNL; itNL != NodeList.end(); itNL++)
		{
			cout<<"cannot calculate variance from the last Time Slice!"<<endl;
		}
	}
	else {
		for (itNL; itNL != NodeList.end(); itNL++)
		{
			itNL->baCalc_variance_std();
		}
	}
}

void TimeSlice::optAllNodes_variance_sim(){
	list<Node>::iterator itNL;
	itNL = NodeList.begin();
