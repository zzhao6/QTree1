...
double fairStrike_simVarianceSwap_Heston_ZL(Para para){

	double 	kappa = para.Kappa,
			rho = para.Rho,
			sigma_V = para.VolOfVol,
			tau = para.Maturity,
			n = para.Steps,
			theta = para.Theta,
			V0 = para.Vol0,
	...
	double a = kappa - 2 * rho * sigma_V;
	double b = sqrt( a * a - 2 * sigma_V *sigma_V );
	double g = ( a / sigma_V ) * ( a / sigma_V ) - 1 + ( a / sigma_V ) * sqrt( (a/sigma_V) *(a/sigma_V) -2);

	double deltaT = tau / n;

	double C_deltaT = r * deltaT + (kappa*theta)/(sigma_V*sigma_V)*( (a+b)*deltaT -2*log((1-g*exp(b*deltaT))/(1-g))  );
	double D_deltaT = (a+b)/(sigma_V*sigma_V)*((1-exp(b*deltaT))/(1-g*exp(b*deltaT)));

	double f_V0 = exp(C_deltaT+D_deltaT*V0) + exp(-r*deltaT) -2;

	double result=0.0; result += f_V0;
	for (int i=2;i<=n;i++){
		double c_i = (2*kappa)/(sigma_V*sigma_V*(1-exp(-kappa*deltaT*(i-1))));
		double f_i_V0 = exp(C_deltaT+(c_i*exp(-kappa*deltaT*(i-1)))/(c_i-D_deltaT)*D_deltaT*V0)
						*pow((c_i)/(c_i-D_deltaT),(2*kappa*theta)/(sigma_V*sigma_V))
						+exp(-r*deltaT)-2;
		result += f_i_V0;
	}

	result = result * exp(r*deltaT)/tau*10000;
